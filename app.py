...
REMOTE_LOG_FILE = "remote_log.json"
...
def load_remote_logs():
    global remote_logs
    if os.path.exists(REMOTE_LOG_FILE):
        try:
            with open(REMOTE_LOG_FILE, 'r', encoding='utf-8') as f:
                remote_logs = json.load(f)
        except: remote_logs = []
...
def save_remote_logs():
    try:
        with open(REMOTE_LOG_FILE, 'w', encoding='utf-8') as f:
            json.dump(remote_logs[-500:], f, ensure_ascii=False, indent=2)
    except: pass
...
@app.before_request
def protect_routes():
    if request.path in ["/", "/kick", "/troll", "/payload", "/remote"] or request.path.startswith("/api"):
        check_ip()
...
<style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:1.5rem;transition:all .4s;position:relative;overflow:hidden;display:flex;flex-direction:column;}
    ...
    .player-chat-log{background:#0f172a;border:1px solid var(--border);border-radius:12px;padding:8px;height:120px;overflow-y:auto;margin-top:auto;font-family:'JetBrains Mono',monospace;font-size:0.8rem;flex-grow:1;}
    .chat-entry{margin:2px 0;color:#94a3b8;}
    .chat-entry span{color:var(--primary);}
    .remote-log{background:#0f172a;border:1px solid var(--border);border-radius:12px;padding:10px;height:500px;overflow-y:auto;font-family:'JetBrains Mono',monospace;font-size:0.85rem;}
    .remote-entry{background:#1e293b;padding:10px;border-radius:8px;margin:8px 0;}
    .remote-path{font-weight:bold;color:var(--primary);cursor:pointer;}
    .remote-args{color:#94a3b8;margin:6px 0;white-space:pre-wrap;}
    .copy-btn{padding:6px 12px;background:var(--primary);color:black;border:none;border-radius:8px;cursor:pointer;font-size:0.8rem;margin-left:8px;}
    ...
    .modal-content{background:var(--card);border:2px solid var(--primary);border-radius:16px;width:90%;max-width:900px;padding:2rem;box-shadow:0 30px 80px rgba(6,182,212,.5);}
...
<div class="main">
    <div class="sidebar">
        ...
        <div class="nav-item" data-tab="logger">Logger</div>
    ...
    <div class="content">
        ...
        <div id="logger-tab" class="tab" style="display:none;">
            <button class="btn" style="margin-bottom:20px;" onclick="document.getElementById('remoteSpyModal').classList.add('active')">Remote Spy</button>
            <div class="remote-log" id="remoteLog"></div>
        </div>
...
<!-- Remote Spy Modal -->
<div class="modal" id="remoteSpyModal"><div class="modal-content">
    <h2>Remote Spy</h2>
    <div class="remote-log" id="remoteSpyLog"></div>
    <div class="modal-buttons">
...
document.querySelectorAll('.nav-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (item.dataset.tab === 'logger') loadRemoteLogs();
    });
});
...
socket.on("player_chat", data => {
    const card = document.getElementById(`card_${data.userid}`);
    if (!card) return;
    let chatLog = card.querySelector(".player-chat-log");
    if (!chatLog) {
        chatLog = document.createElement("div");
        chatLog.className = "player-chat-log";
        card.appendChild(chatLog);
    }
    const entry = document.createElement("div");
    entry.className = "chat-entry";
    entry.innerHTML = `<span>[${data.time}] ${data.username}:</span> ${data.message}`;
    chatLog.appendChild(entry);
    chatLog.scrollTop = chatLog.scrollHeight;
});

// Remote Spy
socket.on("remote_log", log => {
    const logDiv = document.getElementById("remoteSpyLog");
    const entry = document.createElement("div");
    entry.className = "remote-entry";
    entry.innerHTML = `
        <div><strong>[${log.time}] ${log.username}</strong></div>
        <div class="remote-path" onclick="navigator.clipboard.writeText('${log.path}')">${log.path} <button class="copy-btn">Copy Path</button></div>
        <div class="remote-args">${log.args}</div>
        <button class="copy-btn" onclick="navigator.clipboard.writeText(\`${log.fire}\`)">Copy FireServer</button>
    `;
    logDiv.appendChild(entry);
    logDiv.scrollTop = logDiv.scrollHeight;
});

function loadRemoteLogs() {
    fetch("/remote").then(r => r.json()).then(data => {
        const logDiv = document.getElementById("remoteSpyLog");
        logDiv.innerHTML = "";
        data.forEach(log => {
            const entry = document.createElement("div");
...
function render(data){
    ...
    Object.entries(data.players).forEach(([id,p])=>{
        ...
        card.innerHTML=`
            ...
            <div class="player-chat-log" id="chat_${id}"></div>
        `;
    });
...
def remote_spy():
    check_ip()
    if request.method == "GET":
        return jsonify(remote_logs[-100:])
    else:
        data = request.get_json() or {}
        if data.get("type") == "chat":
            socketio.emit("player_chat", {
            ...
            fire_code = f"local args = {json.dumps(args, ensure_ascii=False)}\n{path}:FireServer(unpack(args))"
            log_entry = {
                "time": datetime.now().strftime("%H:%M:%S"),
                "userid": userid,
                "username": username,
            ...
            remote_logs.append(log_entry)
            save_remote_logs()
            socketio.emit("remote_log", log_entry)
        return jsonify({"ok": True})
