...
.nav-btn {background:none; border:none; color:#aaa; padding:15px; text-align:left; font-size:1.1rem; cursor:pointer; border-radius:12px; transition:0.3s; margin-bottom:12px; font-weight:600; font-family:Inter}
...
.history-item {background:rgba(30,30,30,0.5); padding:18px; border-radius:12px; border-left:5px solid #444; display:flex; justify-content:space-between; align-items:center; font-family:Inter}
.history-item.connect {border-color:#00ffaa; background:rgba(0,255,170,0.05)}
.history-item.disconnect {border-color:#ff3366; background:rgba(255,51,102,0.05)}
.history-item.action {border-color:#ffcc00; background:rgba(255,204,0,0.05)}
.history-time {color:#555; font-size:0.9rem; font-family:monospace}
    ...
    <div id="historyPage" class="page">
        <div class="container">
            ...
            <div style="text-align:right; margin-bottom:20px;">
                <button class="kick-btn" style="background:#444; width:150px" onclick="clearHistory()">Clear Logs</button>
            </div>
...
function addLog(msg, type) {
    ...
    item.innerHTML = `<strong>${msg}</strong><span class="history-time">${time}</span>`;
    list.prepend(item);
...
function render(data) {
    ...
    Object.entries(data.players).forEach(([id, p]) => {
        ...
        if (!card) {
            ...
            addLog(`Connection: ${p.username} (${id})`, "connect");
        }

        // RECOPIE EXACTE DE TOUS TES BOUTONS ORIGINAUX
        card.innerHTML = `
    ...
    document.querySelectorAll('.card').forEach(c => {
        ...
        if (!currentIds.has(id)) {
            addLog(`Disconnection: User ID ${id}`, "disconnect");
            c.remove();
        }
    });
}
